---
title: Plan: friendlier /attractor run + validate guided UX
date: 2026-02-14
goal: Let's come up with a plan to make the interface for executing attractor from within pi more user friendly. I'm thinking we want to have a menu for selecting/auto-completing workflows, a friendier presentation of the workflow name/description, and forced providing the goal as a standalone step
status: approved
---
...
- Human review accepted the plan direction and explicitly required:
  - workflow `description` as a new schema field,
  - no `--goal` CLI support,
  - resume flow must not require goal and must not allow goal override,
  - workflow discovery scope to remain `.attractor/workflows` for now,
  - `validate` to receive the same guided workflow selection treatment as `run`.
- Final plan revision incorporates all accepted constraints and keeps execution semantics unchanged after setup.